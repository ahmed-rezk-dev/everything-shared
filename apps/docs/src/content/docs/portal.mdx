...
import importedPortalCode from '@/../node_modules/@everything-shared/portal/src/portal.tsx?raw';
...
<Aside>
  On the web, it is recommended to use [createPortal](https://react.dev/reference/react-dom/createPortal) from `react-dom` instead. RN Primitives use this portal primitive for native only. On the web, RN Primitves use radix-ui primitives which uses `createPortal`.
</Aside>
...
```tsx
import { Portal, PortalHost } from '@everything-shared/portal';
